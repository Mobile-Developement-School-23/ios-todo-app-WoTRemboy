...
public final class FileCacheCoreData {
    ...
    func insertToDatabaseCoreData(item: ToDoItem) {
        let context = CoreDataManager.shared.backgroundContext()
        
        context.perform {
            _ = self.createCoreDataItem(from: item, in: context)
            
            do {
                try context.save()
                DDLogDebug("Inserted into database CoreData", level: .debug)
                self.loadFromDatabaseCoreData()
            } catch {
                DDLogError("Inserting into database error: \(error)", level: .error)
            }
        }
    }

    func updateInDatabaseCoreData(item: ToDoItem) {
        let context = CoreDataManager.shared.backgroundContext()
        
        context.perform {
            if let existingItem = self.fetchItem(withID: item.id, in: context) {
                self.updateExistingItem(existingItem, with: item)
                
                do {
                    try context.save()
                    DDLogDebug("Updated in database CoreData", level: .debug)
                    self.loadFromDatabaseCoreData()
                } catch {
                    DDLogError("Updating in database error: \(error)", level: .error)
                }
            }
        }
    }

    func deleteFromDatabaseCoreData(at itemID: String) {
        let context = CoreDataManager.shared.backgroundContext()
        ...
        context.perform {
            if let existingItem = self.fetchItem(withID: itemID, in: context) {
                ...
                do {
                    try context.save()
                    DDLogDebug("Deleted from database CoreData", level: .debug)
                    self.loadFromDatabaseCoreData()
                } catch {
                    DDLogError("Deleting from database error: \(error)", level: .error)
                }
            }
        }
    }
